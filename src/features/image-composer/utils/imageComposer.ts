import type { UploadedImage, ComposedImageSettings, ImageDimensions } from '../types';
  ...
  switch (settings.layout) {
    case 'horizontal':
      ...
      if (settings.sizeMode === 'minimum') {
        ...
        // 各画像の幅を最小高さに合わせて調整
        width = images.reduce((sum, img) => {
          const scale = height / img.height;
          return sum + img.width * scale;
        }, 0) + (images.length - 1) * settings.gap;
      } else {
    ...
    case 'grid':
      ...
      width = columnWidths.reduce((sum, w) => sum + w, 0) + (columns - 1) * settings.gap;
      height = rowHeights.reduce((sum, h) => sum + h, 0) + (rows - 1) * settings.gap;
      break;
  }
...
function calculateImagePositions(
  ...
  switch (settings.layout) {
    ...
    case 'grid':
      ...
      // セルサイズを計算
      const cellWidth = (canvasSize.width - (columns - 1) * settings.gap) / columns;
